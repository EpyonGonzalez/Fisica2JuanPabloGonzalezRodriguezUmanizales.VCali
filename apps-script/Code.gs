...
function onOpen() {
  ...
  ui.createMenu("Fisica II - Taller 2")
    ...
    .addItem("Crear Resumen", "crearResumen")
    .addToUi();
}
...
function crearPortada() {
  ...
  hoja.getRange(fila, 1).setValue("Juan Pablo Gonzalez Rodriguez")
    .setFontSize(14)
    .setHorizontalAlignment("center");
  fila += 4;
...
/**
 * Crea la hoja de resumen con los resultados de todos los ejercicios.
 */
function crearResumen() {
  ...
  hoja.clear();
  hoja.clearFormats();
  hoja.setColumnWidths(1, 4, 180);

  let fila = 1;

  hoja.getRange(fila, 1, 1, 4).merge()
    .setValue("RESUMEN DE RESULTADOS - TALLER II FISICA II")
    .setFontSize(14)
    .setFontWeight("bold")
  ...
  const resumen = [
    ["Ejercicio", "Tema", "Resultado Principal", "Unidad"],
    ["EJ1", "Onda Sonora", "s_max = 1.16 x 10^-8", "m"],
    ["EJ2", "Efecto Doppler", "f' = 568.2", "Hz"],
    ["EJ3", "Espejo Concavo", "di = 100 cm, M = -1", "cm"],
    ["EJ4", "Espejo Plano", "Imagen virtual simetrica", "-"],
    ["EJ5", "Doble Rendija", "Ancho central = 16.0", "mm"],
    ["EJ6", "Lentes Delgadas", "di = 60 cm, M = -2", "cm"]
  ];

  hoja.getRange(fila, 1, resumen.length, 4).setValues(resumen)
  ...
  hoja.getRange(fila + 1, 1, resumen.length - 1, 4)
    .setBackground(COLORES.resultado);

  fila += resumen.length + 2;

  hoja.getRange(fila, 1, 1, 4).merge()
    .setValue("Universidad de Manizales - Fisica II - Juan Pablo Gonzalez Rodriguez")
    .setFontSize(10)
    .setFontStyle("italic")
    .setHorizontalAlignment("center");
}
